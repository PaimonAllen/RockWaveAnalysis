...
data_train = np.loadtxt(
    "/home/tzr/DataLinux-SSD/Dataset/2.isCrackClassification/KNN/dataset/train10/train10.txt", delimiter=",")
data_test = np.loadtxt(
    "/home/tzr/DataLinux-SSD/Dataset/2.isCrackClassification/KNN/dataset/train10/test10.txt", delimiter=",")
...
# 数据分割
train, labels_train = np.split(data_train,  # 要切分的数组
                               (4,),  # 沿轴切分的位置，第7列开始往后为y
                               axis=1)  # 1代表纵向分割，按列分割

train = train[:, 0:4]

test, labels_test = np.split(data_test,  # 要切分的数组
                             (4,),  # 沿轴切分的位置，第7列开始往后为y
                             axis=1)  # 1代表纵向分割，按列分割
...
def KNNclassify():
    ...
    m = len(labels_test)
    print("len(y_test):", len(labels_test))
    for i in range(0, m):
        # print(i)
        # # 获得文件的名字
        # fileNameStr = testFileList[i]
        # 获得分类的数字
        classNumber = int(labels_test[i])
        # print(classNumber)
        # 获得测试集的1x1024向量,用于训练
        # print(testMatPAC[i])
