...
class Fea_Extra():
    ...
    def Fre_fea(self, signal_):
        ...
        f_fea = np.array([f_16, MSF, f_18, RVF])
...
FileList = listdir('/home/tzr/DataLinux-SSD/Dataset/7.onlyShapeClassification/KNN/dataset/train11/')
# 返回文件夹下文件的个数
m = len(FileList)
# print(FileList)
print(m)
for i in range(m):
    ...
    y1 = np.loadtxt(f"/home/tzr/DataLinux-SSD/Dataset/7.onlyShapeClassification/KNN/dataset/train11/{fileNameStr}")
    print(np.max(y1))
    print(y1.shape)
    # print(max(y1))
    ...
    print(X.Fre_fea(y1))
    saveArr1 = X.Time_fea(y1)
    saveArr2 = X.Fre_fea(y1)
    saveArr = np.append(saveArr1, saveArr2)
    saveArr = np.append(saveArr, classNumber)
    # saveArr = [max_value, min_value, mean_value,range_value,classNumber]
    
    saveArr = np.array(saveArr, dtype=float).reshape(1, 18)
    print(saveArr)
    # print(saveArr.ndim)
    # saveClassName = np.array([1], dtype=int)
    # sys.exit()
    with open("/home/tzr/DataLinux-SSD/Dataset/7.onlyShapeClassification/KNN/dataset/train12/train-all.txt", 'a+') as f:
