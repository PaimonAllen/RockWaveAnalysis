...
FileList = listdir('/home/tzr/DataLinux/Documents/GitHubSYNC/RockWaveAnalysis/2.isCrackClassification/KNN/dataset/test3/')
# 返回文件夹下文件的个数
m = len(FileList)
# print(FileList)
print(m)
for i in range(m):
    ...
    # 'a+') as f:
    data = np.loadtxt(f"/home/tzr/DataLinux/Documents/GitHubSYNC/RockWaveAnalysis/2.isCrackClassification/KNN/dataset/test3/{fileNameStr}")
    # print(data)
    max_value = max(data)
    min_value = min(data)
    ...
    print(saveArr.ndim)
    # saveClassName = np.array([1], dtype=int)
    with open("/home/tzr/DataLinux/Documents/GitHubSYNC/RockWaveAnalysis/2.isCrackClassification/SVM/dataset/test5/test5.txt", 'a+') as f:
        np.savetxt(f, saveArr, delimiter=',')
        # # np.savetxt(f,)
        # np.savetxt(f, saveClassName)
