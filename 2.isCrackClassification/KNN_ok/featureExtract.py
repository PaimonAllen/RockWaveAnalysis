...
for i in range(m):
    ...
    #     # with open("./dataset/rockshape/features_extract/test1.csv", 'a+') as f:
    data = np.loadtxt(f"/home/tzr/DataLinux-SSD/Dataset/6.crackLengthAndShapeClassification/KNN/dataset/train2/{fileNameStr}")
    # print(data)
    max_value = max(data)
    min_value = min(data)
    ...
    print(saveArr.ndim)
    # saveClassName = np.array([1], dtype=int)
    with open("/home/tzr/DataLinux-SSD/Dataset/6.crackLengthAndShapeClassification/KNN/dataset/train10/train10.txt", 'a+') as f:
        np.savetxt(f, saveArr, delimiter=',')
        # # np.savetxt(f,)
        # np.savetxt(f, saveClassName)
