...
def SignalClassTest():
    ...
    trainingFileList = listdir('../../KNN/dataset/train2/')
    # 返回文件夹下文件的个数
    ...
    for i in range(m):
        ...
        trainingMat[i, :] = vector(
            '../../KNN/dataset/train2/%s' % (fileNameStr))
    clf = SVC(C=0.5, kernel='rbf')
    clf.fit(trainingMat, Labels)
    # 返回testDigits目录下的文件列表
    testFileList = listdir('../../KNN/dataset/test2/')
    # 错误检测计数
    errorCount = 0.0
    # 测试数据的数量
    ...
    for i in range(mTest):
        ...
        classNumber = int(fileNameStr.split('-')[1])
        # 获得测试集的1x1024向量,用于训练
        vectorUnderTest = vector('../../KNN/dataset/test2/%s' % (fileNameStr))
        # 获得预测结果
